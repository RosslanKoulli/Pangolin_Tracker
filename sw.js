...
self.addEventListener('fetch', (event) => {
    ...
    if (isApiRequest(url)) {
        ...
        // Map tiles: Cache-first (cache tiles user has viewed before)
...
/**
 * Check if request is for map tiles (OpenStreetMap)
 */
 ...
 * Caches tiles user has viewed for offline use
 */
async function cacheFirstMapTile(request) {
    ...
    try {
        ...
        // Return a transparent placeholder tile when offline
        console.log('[SW] Map tile unavailable offline:', request.url);
        return new Response(
            // 1x1 transparent PNG
